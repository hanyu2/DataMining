...
for idxfiles = 1:numfiles
    trainMatrix_MR = sprintf('trainingMatrix_MindReading%d.mat', idxfiles);
    trainLabels_MR = sprintf('trainingLabels_MindReading_%d.mat',idxfiles);
    testMatrix_MR = sprintf('testingMatrix_MindReading%d.mat',idxfiles);
    testLabels_MR = sprintf('testingLabels_MindReading%d.mat',idxfiles);
    unlabeledMatrix_MR = sprintf('unlabeledMatrix_MindReading%d.mat',idxfiles);
    unlabeledLabels_MR = sprintf('unlabeledLabels_MindReading_%d.mat',idxfiles);
    load(trainMatrix_MR);
    load(trainLabels_MR);
    load(testMatrix_MR);
    load(testLabels_MR);
...
avg = avg./3;
figure(2);
title('MindReading with Uncertainty-based Sampling')
hold on;
plot(accuracies(:,1),'-k');
plot(accuracies(:,2),'-m');
plot(accuracies(:,3),'-c');
